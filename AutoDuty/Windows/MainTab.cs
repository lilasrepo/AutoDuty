        ...
        internal static void Draw()
        {
                ...
                else
                {
                    ...
                    using (ImRaii.Disabled(!VNavmesh_IPCSubscriber.IsEnabled || !InDungeon || !VNavmesh_IPCSubscriber.Nav_IsReady || !BossMod_IPCSubscriber.IsEnabled))
                    {
                        ...
                        if(dutyMode == DutyMode.Variant)
                        {
                            ...
                            using var __ = ImRaii.Disabled(AutoDuty.Configuration.Meta.AutoDutyModeEnum == AutoDutyMode.Playlist);
                            ImGui.SameLine();
            ...
            else
            {
                ...
                using (ImRaii.Disabled(Plugin.States.HasFlag(PluginState.Looping)))
                {
                    switch (AutoDuty.Configuration.Meta.AutoDutyModeEnum)
                    {
                        case AutoDutyMode.Looping:
                        {
                            ...
                            if (AutoDuty.Configuration.Meta.DutyModeEnum != DutyMode.None)
                            {
                                ...
                                // porting-note(api13): restored via gap-filled ReaderGCArmyMemberList. Defaults OFF —
                                // the member-select path reads the GcArmyMemberList AtkValue layout, which is runtime-
                                // unverified on TC game 7.20; enabling it falls back cleanly to the proven flow if unticked.
                    ...
                    {
                        using ImRaii.IEndObject _ = ImRaii.Disabled(Plugin.States != PluginState.None);

                        ImGui.PushItemWidth(ImGui.GetContentRegionAvail().X - 180 * ImGuiHelpers.GlobalScale);
                        ImGui.SetItemAllowOverlap();
                        using (ImRaii.IEndObject configCombo = ImRaii.Combo("##PlaylistCombo", Plugin.PlaylistCurrent.Name))
                        {
                            if (configCombo)
