...
internal static class CrucibleItemData
{
    ...
    // shipped with TC game 7.20, so the name lookups are inert. Crucible content itself is absent on
    // 7.20 (ContentType 40 / XBM* addons never resolve), so nothing reaches these at runtime.
    public static string NameOf(uint row) => $"item {row}";

    public static uint ItemIn(string text) => 0;
