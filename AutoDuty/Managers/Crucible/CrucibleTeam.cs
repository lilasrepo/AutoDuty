    ...
    internal static unsafe class CrucibleTeam
    {
        ...
        private static SortedDictionary<uint, string> SheetNames
        {
            get
            {
                ...
                // TODO(api13): api13 Lumina's XBMPet has no Pet-link column (only Unknown0) and the sheet is
                // empty on TC game 7.20 anyway; Crucible familiar names stay empty. Pet sheet kept for parity.
                _ = pets;
                return sheetNames;
            }
        }
        ...
        {
            // TODO(api13): XBMManager (Crucible, game 7.5) is absent from api13 FFXIVClientStructs; fall back
            // to the cached familiar list, which is empty on TC game 7.20.
            IReadOnlyDictionary<uint, CrucibleFamiliar> cached = Familiars;

            return SheetNames.Keys.Where(cached.ContainsKey);
        }
        ...
        public static bool OwnershipKnown
        {
            get
            {
                // TODO(api13): XBMManager absent from api13 FFXIVClientStructs (see Owned()).
                return false;
            }
        }
