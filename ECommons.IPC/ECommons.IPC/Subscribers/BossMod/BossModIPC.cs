...
public sealed class BossModIPC : IPCBase
{
    ...
    // porting-note: walk-back ECommons EzIPC.Init only handles Action / Func<...> generic delegates;
    // a custom delegate type (AddTransientStrategyDelegate) hits GetGenericTypeDefinition() on a
    // non-generic Type and throws. Convert to Func<...,bool> with the same signature.
    [EzIPC("Presets.AddTransientStrategy")] public Func<string, string, string, string, bool> Presets_AddTransientStrategy { get; private set; }
}