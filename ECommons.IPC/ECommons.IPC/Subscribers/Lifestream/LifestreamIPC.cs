...
public sealed class LifestreamIPC : IPCBase
{
    ...
    [EzIPC("Teleport")]
    public Func<uint, byte, bool> Teleport { get; private set; }
    ...
    public Action<PropertyType, HouseEnterMode?> EnqueuePropertyShortcut { get; private set; }
    ...
    [EzIPC("MoveEx")]
    public Action<List<Vector3>, bool?, float?, float?> MoveEx { get; private set; }
    ...
    public Action MoveToWorkshop { get; private set; }
    ...
    // Un-stubbed 2026-08-30: the old TODO said "AutoDuty doesn't use these IPC endpoints
    // anyway" -- upstream AutoDuty now calls ChangeCharacter from IPCSubscriber.cs. The
    // ErrorCode enum the stub was waiting on is present (ECommons/ErrorCode.cs gap-fill).
    [EzIPC("ChangeCharacter")]
    public Func<string, string, ECommons.ErrorCode> ChangeCharacter { get; private set; }

    // Logout stays stubbed: still unused by AutoDuty.
    // [EzIPC]
